...
const PatientDashboard = () => {
  ...
  const getInitialChartFor = (name) => {
    return Array.from({ length: 32 }).reduce((acc, _, i) => {
      ...
      if (name === 'John Doe') {
        ...
        if (toothNum === 8 || toothNum === 9 || toothNum === 24)
          acc[toothNum] = 1;
        else if (toothNum === 2 || toothNum === 3)
          acc[toothNum] = 2;
        else
          acc[toothNum] = 0;
      } 
  ...
  const patientProfiles = {
    'John Doe': {
      appointment: 'June 30 at 10:00 AM',
      advice: 'Avoid cold drinks.'
    },

    'Jane Smith': {
      appointment: 'July 05 at 2:30 PM',
      advice: 'Brush gently.'
    },

    'Alex Mercer': {
      appointment: 'July 12 at 9:15 AM',
      advice: 'Floss daily.'
    }
  };
  ...
  const handlePatientChange = (e) => {
    ...
    setCurrentTeethStates(
      getInitialChartFor(next)
    );

    setCurrentProfile(
      patientProfiles[next]
    );
  };


  const healthyCount = Object.values(currentTeethStates)
    .filter((s) => s === 0).length;

  const cavityCount = Object.values(currentTeethStates)
    .filter((s) => s === 1).length;

  const filledCount = Object.values(currentTeethStates)
    .filter((s) => s === 2).length;
      ...
      <main className="flex-1 p-6">
        ...
        {activeTab === 'report' && (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl space-y-2 shadow">


              <p>
                <b>Patient:</b> {selectedPatient}
              </p>


              <p>
                <b>Healthy Teeth:</b> {healthyCount}
              </p>


              <p>
                <b>Cavities:</b> {cavityCount}
              </p>


              <p>
                <b>Filled Teeth:</b> {filledCount}
              </p>


              <p>
                <b>Appointment:</b> {currentProfile.appointment}
              </p>


              <p>
                <b>Advice:</b> {currentProfile.advice}
              </p>
